...
{\mbox{\kern.06em\vrule height.3ex}\vbox{\hrule width#1}\hbox{\vrule height.3ex}}
...
\item This package pretends that \texttt{physics} package is loaded so that this package won't be overriden if \verb|\@onefilewithoptions{physics}| is called afterward (e.g. by \verb|\usepackage{physics}|) and packages loaded afterward that checks whether \texttt{physics} is loaded to determine its behavior (e.g. \texttt{siunitx}) work correctly. To disable this, use the \texttt{nopretend} option (not recommended).
...
\paragraph*{In the commands listed below, the left column is long-form names with non-default alternate names (if any), the middle column is default shorthand commands with detailed syntaxes and explanations.}
\paragraph*{Commands that have different definitions come with \texttt{PT} in the beginning of their name (e.g. \verb|\PTmqty|). If \texttt{nooverride} is not used or the \texttt{physics} package is not loaded before this package, the commands without \texttt{PT} will be silent overriden to be the same as the ones with \texttt{PT}.}
\subsection{Automatic bracing}
\begin{longtable}[l]{ l l p{6cm} }
\verb|\PTquantity| or \verb|\PPquantity| or \verb|\PHqty| & \verb|\PTqty(\typical)| $\displaystyle\rightarrow \PTqty(\typical)$ & automatic $\PTqty(\;)$ braces \\
...
The default del symbol $\vnabla$ used in \texttt{physics} vector notation can be switched to appear with an arrow $\vec{\vnabla}$ by including the option \texttt{arrowdel} in the document preamble $\rightarrow$ \verb|\usepackage[arrowdel]{physics-patch}|.
\begin{longtable}[l]{ l l p{6cm} }
...
The default differential symbol $\dd$ which is used in \verb|\differential| and \verb|\derivative| can be switched to an italic form $d$ by including the option \texttt{italicdiff} in the preamble $\rightarrow$ \verb|\usepackage[italicdiff]{physics-patch}|.
\begin{longtable}[l]{ l l p{6cm} }
...
\end{displaymath}
\end{minipage}
The extra \verb|\PTmqty| groups were required in this case in order to get the $a$ and $b$ elements to behave as a single element, since \verb|\PTmqty{\imat{2}}| also acts like a single matrix element (the same can be said of the grouped $c$ and $d$ elements). Finally, the outermost \texttt{pmatrix} environment could have also been replaced with the \texttt{physics} macro \verb|\PTmqty()|, allowing the above example to be written on one line:
\begin{minipage}{9cm}
\begin{verbatim}
...
& \verb|\PTsmqty(\imat{3})| \rightarrow \PTsmqty(\imat{3}) & formatted with \verb|\PTmqty| or \verb|\PTsmqty| \\
\verb|\PTxmatrix| & \verb|\PTxmat{x}{n}{m}| & elements of $n \times m$ matrix filled with $x$, if not provided, $1$ is used \\
& \verb|\PTxmat{x}{3}{3}| \rightarrow \PTxmat{x}{3}{3} & example \\
& \verb|\PTxmat{x}{}{3}| \rightarrow \PTxmat{x}{}{3} & example for $n$ not provided \\
& \verb|\PTxmat{x}{3}{}| \rightarrow \PTxmat{x}{3}{} & example for $m$ not provided \\
& \verb|\PTxmat*{x}{n}{m}| & star for element indices, skip row/column indices $n=1$/$m=1$ \\
& \verb|\PTxmat*{x}{3}{3}| \rightarrow \PTxmat*{x}{3}{3} & example \\
& \verb|\PTxmat*{x}{1}{3}| \rightarrow \PTxmat*{x}{1}{3} & example for $n=1$ \\
& \verb|\PTxmat*{x}{3}{1}| \rightarrow \PTxmat*{x}{3}{1} & example for $m=1$ \\
& \verb|\PTxmat{x}{n}{m}[p]| & only show $p$ rows (including $\vdots$ row) with skipped rows indicated by $\vdots$. If $n$ isn't provided, $p$ is used \\
& \verb|\PTxmat{x}{5}{3}[3]| \rightarrow \PTxmat{x}{5}{3}[3] & example \\
& \verb|\PTxmat{x}{3}{3}[3]| \rightarrow \PTxmat{x}{3}{3}[3] & example for $n=p$ \\
& \verb|\PTxmat{x}{}{3}[3]| \rightarrow \PTxmat{x}{}{3}[3] & example for $n$ not provided \\
& \verb|\PTxmat{x}{n}{m}[p][q]| & only show $p$ rows (including $\vdots$ row) and $q$ columns (including $\ldots$ column) with skipped rows indicated by $\vdots$, skipped columns indicated by $\ldots$, intersection of $\vdots$ row and $\ldots$ column being $\ddots$. If $n$/$m$ isn't provided, $p$/$q$ is used \\
& \verb|\PTxmat{x}{5}{5}[3][3]| \rightarrow \PTxmat{x}{5}{5}[3][3] & example \\
& \verb|\PTxmat{x}{5}{3}[3][3]| \rightarrow \PTxmat{x}{5}{3}[3][3] & example for $m=q$ \\
& \verb|\PTxmat{x}{3}{3}[3][3]| \rightarrow \PTxmat{x}{3}{3}[3][3] & example for $n=p$ and $m=q$ \\
& \verb|\PTxmat{x}{5}{}[3][3]| \rightarrow \PTxmat{x}{5}{}[3][3] & example for $m$ not provided \\
& \verb|\PTxmat{x}{}{}[3][3]| \rightarrow \PTxmat{x}{}{}[3][3] & example for $n$ and $m$ not provided \\
& \verb|\PTxmat*{x}{n}{m}{g}| & customize last row's element indices to $g$ \\
& \verb|\PTxmat*{x}{3}{3}{A}| \rightarrow \PTxmat*{x}{3}{3}{A} & example \\
& \verb|\PTxmat*{x}{5}{5}[3][3]{A}| \rightarrow \PTxmat*{x}{5}{5}[3][3]{A} & example with $p$ and $q$ \\
& \verb|\PTxmat*{x}{n}{m}{g}{h}| & customize last row's element indices to $g$ and last column's element indices to $h$ \\
& \verb|\PTxmat*{x}{3}{3}{A}{B}| \rightarrow \PTxmat*{x}{3}{3}{A}{B} & example \\
& \verb|\PTxmat*{x}{5}{5}[3][3]{A}{B}| \rightarrow \PTxmat*{x}{5}{5}[3][3]{A}{B} & example with $p$ and $q$ \\
& \verb|\PTxmat[0 or 1 or 2]{x}{n}{m}[p][q]| & Change the $\vdots$ row/$\ldots$ column from the second last one to last one, 0 for both, 1 for row only, 2 for column only. Only work when corresponding $p$/$q$ is provided and do not change the behavior of element indices \\
& \verb|\PTxmat[0]{x}{5}{5}[3][3]| \rightarrow \PTxmat[0]{x}{5}{5}[3][3] & example \\
& \verb|\PTxmat[1]{x}{5}{5}[3][3]| \rightarrow \PTxmat[1]{x}{5}{5}[3][3] & example \\
& \verb|\PTxmat[2]{x}{5}{5}[3][3]| \rightarrow \PTxmat[2]{x}{5}{5}[3][3] & example \\
\verb|\zeromatrix| & \verb|\zmat{n}{m}| & $n \times m$ matrix filled with zeros, equivalent to \verb|\xmat{0}{n}{m}| \\
& \verb|\zmat{2}{2})| \rightarrow \zmat{2}{2} & example \\
\verb|\paulimatrix| & \verb|\pmat{n}| & $n^\text{th}$ Pauli matrix \\
& \verb|\PTsmqty(\pmat{0})| \rightarrow \PTsmqty(\pmat{0}) & $n\in \lbrace 0,1,2,3$ or $x,y,z \rbrace$ \\
& \verb|\PTsmqty(\pmat{1})| \rightarrow \PTsmqty(\pmat{1}) & \\
& \verb|\PTsmqty(\pmat{2})| \rightarrow \PTsmqty(\pmat{2}) & \\
& \verb|\PTsmqty(\pmat{3})| \rightarrow \PTsmqty(\pmat{3}) & \\
...
& \verb|\PTmqty(\dmat{1,2&3\\4&5})| \rightarrow \PTmqty(\dmat{1,2&3\\4&5}) & \parbox{6cm}{enter matrix elements for each block as a single diagonal element} \\
